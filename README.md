...
| 页脚访客统计 | `counter`（`enabled: false` 即完全关闭） |

注意：条目之间用英文逗号 `,` 隔开，最后一条后不要加逗号；字符串内如出现英文双引号需写成 `\"`。

## 访客统计

页脚右下角显示"访客 / 访问"两个数字，数据来自第三方服务**不蒜子**（GitHub Pages 是纯静态托管，站点自身无法计数）。

- 计数从启用当天的 0 开始，历史访问无法追溯。
- 数字按域名统计。本站域名是 `ljiangang412-ai.github.io`，如果同一账号下还有别的 GitHub Pages 项目，它们会共用同一个计数。
- 服务不可用时，页脚这一行会**自动整行隐藏**，不会显示 0 或空白。
- 在 `data.js` 中把 `counter.enabled` 改成 `false` 即可彻底关闭，连第三方脚本都不会加载。

`counter.provider` 有两个可选值，对应两家**互相独立**的免费服务（数据不互通，换一家等于从 0 重新开始）：

| 值 | 服务 | 说明 |
|---|---|---|
| `"ibruce"`（默认） | `busuanzi.ibruce.info` | 原版，作者 Bruce，运行多年 |
| `"busuanzi.cc"` | `cdn.busuanzi.cc` | CoolCat 的另一套实现，版本较新 |

两者都是个人维护的免费服务，不保证长期可用；若某天数字不再显示，改一下 `provider` 换另一家即可。
